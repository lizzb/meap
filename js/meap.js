/*!
 * Plum v1.5: A library of tools for jQuery..........from meap2 clusterfuck
 *
...
(function ($, undefined) {
	...
	plum.shop.prototype = {
		// The initial cart.
		// This contains an empty expiration time... 
		cart: {
			items: [],
			timeout: null,
		...
		options: {
			...
			// '$00,000.00' or 'â‚¬ 00 000,00' or 'â‚¬000' or any variation of
...
/* ----------------------------------------------------*/
	...
	encodeJSON: function (object)
	{
		...
		case 'number':
			if (isNaN(object)) { throw new Error(); }
			return object;
	...
	escapeJSONString: function (string) {
		 ...
		 * can instead be displayed as "â‚¬1 000,00" by changing "currencyformat"
		 * to "â‚¬0 000,00". The format should be defined as the maximum amount
